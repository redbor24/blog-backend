...
## Что полезного сделано
- Название в заголовке ведёт на главную страницу блога.
- Кнопка "READ ME" под каждым постом ведёт на сам пост.
- Создан шаблон `base.html`, на основе которого построен шаблон `index.html`.
- Шаблоны `blog-details.html` и `contact.html` переделаны с использованием
базового шаблона `base.html`.
